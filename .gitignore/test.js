...
function getApi(weatherUrl, cityName){
...
//test
let LocalStorageEl = localStorage.setItem('name', []);
...
//using ES6 functions now
const renderHistory = () =>{
    const storageCity = (localStorage.getItem('name'));   
    
    storageCity.forEach( storageCity=> {
        
        const renderBtnEl = `<button class="btn btn-secondary container p-2 m-2">${storageCity.value}</button>` 
        renderBtnEl.addEventListener('submit', function(event){
            getApi(weatherUrl,cityName);
            event.preventDefault();})


        leftbarEl.appendChild(renderBtnEl)
    });
